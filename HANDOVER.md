...
- ✅ Phase 4 実装完了（clone 統合コマンド、URL一発で全工程実行）
- ✅ コードベース分割（main.py = CLI登録のみ、`src/` に各機能を分離）
...
- ⬜ 通しテスト（fal.ai 課金後、`clone <URL>` で初のクローン動画生成）
...
uv run main.py clone <URL>                          # 全工程を一気通貫で実行
```

### clone のオプション
- `-n / --frames` 解析フレーム数=カット数（デフォ10）
- `--voice` TTS声（デフォ nova）
- `--character-quality` low/medium/high（デフォ medium）
- `--clip-duration` 各クリップ秒数（デフォ5）
- `--clip-resolution` 480p/720p/1080p（デフォ 1080p）
- `--skip-clips` クリップ生成スキップ（fal.ai課金待ち時のテスト用）
- `--skip-compose` 最終合成スキップ
...
- 解除後の単発テスト：`uv run main.py generate-clip cache/analysis/筋トレ.json -i 0`（5秒1本=約40円）

### 2. `clone <URL>` で初の通しテスト
- `uv run main.py clone <URL>` で download〜compose まで一気通貫
- 解析結果10カット分 generate-clip 実行（合計約400円）
- 失敗動画は判定なし全部出す（要件A、コード側でも catch 済み）
- 出力：`output/<stem>.mp4` が初のクローン動画
...
- `main.py` — CLI エントリポイント（typer 登録のみ、約30行）
- `src/__init__.py`
- `src/ffmpeg_utils.py` — ffmpeg ヘルパー（duration / フレーム抽出）
- `src/download.py` — yt-dlp ダウンロード
- `src/transcribe.py` — Whisper 書き起こし
- `src/analyze.py` — GPT-4o Vision 構成解析
- `src/generate_character.py` — GPT Image キャラ生成
- `src/generate_clip.py` — fal.ai Seedance 動画生成
- `src/synthesize.py` — OpenAI TTS 音声合成
- `src/compose.py` — HyperFrames 合成
- `src/clone.py` — 全工程統合コマンド
